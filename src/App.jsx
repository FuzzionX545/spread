...
export default function App() {
  ...
  const opcoes = p ? [
    ...
    // ⚡ com Liberação na Hora, a velocidade compra aceitação: quem tem pressa engole a taxa alta
    { rot: "🤑", t: tBase + 0.015, aceita: g.turbo ? (duro ? 0.5 : 0.85) : (duro ? 0.3 : 0.65) },
  ] : [];
  ...
  function rodarMes() {
    ...
    setG((s) => {
      ...
      // custo operacional fixo (equipe, sistema, cobrança) + ⚡ turbo se ligado
      const turboCusto = s.turbo ? TURBO_CUSTO : 0;
      caixa -= OPEX + turboCusto;

      // carta-resumo única do fechamento
      const liquido = recebido + rendeu - custo - OPEX - turboCusto;
      const partes = [];
      ...
      if (turboCusto) partes.push(`⚡ –${fmt(turboCusto)} Liberação na Hora`);
      ...
      // Sorteio PRÓPRIO, separado dos outros eventos, pra não roubar espaço das crises.
      if (s.mes % 2 === 0) {
        const rc = Math.random();
        if (rc < 0.24) { selic = Math.min(0.19, selic + 0.01); fila.push({ emoji: "📈", titulo: "Copom subiu a Selic!", sub: `reunião do mês: ${pctm(selic)}% a.a. — seu custo subiu`, delta: 0, cor: C.amber }); }
        else if (rc < 0.40) { selic = Math.max(0.09, selic - 0.01); fila.push({ emoji: "📉", titulo: "Copom cortou a Selic", sub: `reunião do mês: ${pctm(selic)}% a.a. — custo aliviou`, delta: 0, cor: C.green }); }
      }
      if (r < 0.20) { /* faixa reservada — a Selic saiu do sorteio geral e virou o Copom acima */ }
...
const DICAS = [
  ...
  { e: "🛡️", t: "A proteção do investidor", d: "Cofre apanha primeiro · garantia corta calote · filtro decide quem entra.",
    mais: "O investidor tem três escudos: o SEU cofre apanha primeiro (ele só perde se os R$ 200 mil zerarem), a garantia 🧾 corta o calote em 60%, e o filtro de score decide quem entra na carteira. Quanto mais protegida a carteira, mais fácil e mais barato captar — proteção não é burocracia, é o que faz o dinheiro dos outros topar ficar." },
  ...
  { e: "🏦", t: "Por que eles vêm até VOCÊ?", d: "Todo cliente aqui foi recusado (ou não esperado) pelo banco grande.",
    mais: "O banco grande recusa esses clientes por motivos que o jogo carrega escondidos: sem garantia, empresa nova, sem histórico — e o principal: o pedido é PEQUENO. Analisar R$ 60 mil custa quase o mesmo que analisar R$ 5 milhões, então no ticket pequeno a análise come o lucro e o banco nem olha. Esse é o mercado que sobra — e o seu negócio é atender ele." },
  { e: "⚡", t: "Liberação na Hora", d: "Análise em minutos — CUSTA R$ 3 mil todo mês ligada.",
    mais: "Quem tem pressa (fornecedor cobrando amanhã) paga mais caro pra quem libera em minutos. Ligada, a aceitação da taxa 🤑 sobe de 65% pra 85% (pechincheiro: de 30% pra 50%). O preço: R$ 3 mil saem do caixa TODO mês ligado, mesmo se ninguém pegar empréstimo. Não é ganho garantido — é custo fixo comprando aceitação." },
  { e: "⭐", t: "As estrelas (score)", d: "Mais estrelas = paga certinho.",
    mais: "Chance de calote POR MÊS no mercado normal: ⭐ 12,6% · ⭐⭐ 7,7% · ⭐⭐⭐ 4,5% · ⭐⭐⭐⭐ 2,5% · ⭐⭐⭐⭐⭐ 1,3%. Por isso as taxas que eles aceitam são diferentes (5,2% → 2,3% ao mês). Mas a estrela é só a nota-base (tipo Serasa): garantia 🧾, histórico e folga na renda são sinais SEPARADOS — um ★★★ com garantia rende mais que um ★★★★★ sem. Leia o cliente inteiro, não só a estrela." },
